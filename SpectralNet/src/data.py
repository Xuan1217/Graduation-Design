...
def load_CIFAR_100() -> tuple:
    tensor_transform = tensor_transform = transforms.Compose([transforms.ToTensor()])
    ...
    x_train, y_train = zip(*cifar100_train)
    x_train, y_train = torch.cat(x_train), torch.Tensor(y_train)
    x_train = x_train.reshape(-1, 32, 32, 3) / 255.0
    x_test, y_test = zip(*cifar100_test)
    x_test, y_test = torch.cat(x_test), torch.Tensor(y_test)
